    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (response.ok && result.ok) {
                ...
                console.log(`Attempt ${attempt + 1}/${maxRetries} failed for chat ${chatId}:`, result.description);
            }
...
async function sendFile(chatId, fileId, caption = "", fileType = "document") {
    try {
        const data = {
            chat_id: chatId,
            caption: caption,
        ...
        if (response.ok && result.ok) {
            console.log(`✅ File sent successfully to ${chatId}`);
            return true;
        } else {
            console.error(`❌ Failed to send file to ${chatId}:`, result.description);
            return false;
        }
    } catch (e) {
        console.error(`Error sending file to ${chatId}:`, e);
        return false;
    }
}
...
async function handleMovieRequest(env, message) {
    ...
    if (foundMovie) {
        const movieData = movies[foundMovie];
        const fileId = movieData.file_id;
        const fileType = movieData.file_type || "document";
        
        if (fileId) {
            const caption = `🎬 <b>${foundMovie}</b>\n\n${movieData.description || ''}\n\n📁 Type: ${fileType === "video" ? "🎬 Video" : "📎 Document"}`;
            const sent = await sendFile(chatId, fileId, caption, fileType);
...
async function handleAdd(env, message) {
    ...
    await sendMessage(
        chatId,
        `📁 <b>Upload Movie</b>\n\n` +
        `Send any video/document file with caption:\n` +
        ...
        `• Make sure caption is correct before sending`
    );
}
...
async function handleAdminMovies(env, message) {
    ...
    for (const [movieName, movieData] of Object.entries(movies)) {
        ...
        const fileType = movieData.file_type === "video" ? "🎬 Video" : "📎 Document";
        
        text += `🎬 ${movieName}\n`;
        text += `📁 ${fileType} | 🏷️ ${categoriesStr}\n`;
        text += `📅 Added: ${added}\n`;
        text += "━".repeat(30) + "\n\n";
        count++;
    }
...
• Videos: ${Object.values(movies).filter(m => m.file_type === "video").length}
• Documents: ${Object.values(movies).filter(m => m.file_type !== "video").length}
    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            console.log(`📨 Incoming update at ${timestamp}:`, 
                update.message ? update.message.text || 'File/Media' : 'No message');
            
            // ===================== HANDLE FILE UPLOADS =====================
            if (update.message && (update.message.document || update.message.video || update.message.audio || update.message.animation)) {
                ...
                if (!fileId) {
                    ...
                    await sendMessage(chatId, "❌ Could not get file ID from Telegram");
                    return new Response(JSON.stringify({ ok: false, error: "No file ID" }));
                }
                
                // Check if user is admin
                if (!isAdmin(userId)) {
                    console.log(`❌ Non-admin user ${userId} tried to upload file`);
                    await sendMessage(chatId, "❌ Admin only! Only admin can upload movies.");
                    return new Response(JSON.stringify({ ok: false, error: "Not admin" }));
                }
                
                console.log(`👤 Admin ${userId} uploading file, caption: "${caption}"`);
                
                // Parse caption
                if (!caption || !caption.includes("|")) {
                    console.log("❌ No caption or wrong format");
                    await sendMessage(
                        chatId,
                        "❌ <b>Caption missing or wrong format!</b>\n\n" +
                        "Please include caption with this format:\n" +
                        "<code>Movie Name | Category1, Category2 | Description</code>\n\n" +
                        "Example:\n" +
                        "<code>Inception | Action, Sci-Fi, Thriller | A thief who steals corporate secrets...</code>"
                    );
                    return new Response(JSON.stringify({ ok: false, error: "Invalid caption" }));
                }
                ...
                if (parts.length < 2) {
                    console.log("❌ Not enough parts in caption");
                    await sendMessage(
                        chatId, 
                        "❌ <b>Wrong format!</b>\n\n" +
                        "Correct format:\n" +
                ...
                if (parts.length === 2) {
                    // Format: Movie Name | Description
                    description = parts[1];
                    console.log(`🎬 Movie: ${movieName}, Description: ${description.substring(0, 50)}...`);
                } else {
                    // Format: Movie Name | Category1, Category2 | Description
                    categories = parts[1].split(",").map(cat => cat.trim());
                    description = parts[2];
                    console.log(`🎬 Movie: ${movieName}, Categories: ${categories.length}, Description: ${description.substring(0, 50)}...`);
                }
                ...
                movies[movieName] = {
                    ...
                    added_by: userId,
                    added_by_name: message.from.first_name || "Admin",
                    file_size: message.document ? message.document.file_size : 
                               message.video ? message.video.file_size : 0
                };
                
                const saved = await saveMovies(env, movies);
                
                if (saved) {
                    const categoriesStr = categories.length > 0 ? 
                        categories.join(", ") : "No categories";
                    
                    const successMessage = `✅ <b>Movie Added Successfully!</b>\n\n` +
                        `🎬 <b>Movie:</b> ${movieName}\n` +
                        `📁 <b>Type:</b> ${fileType === "video" ? "🎬 Video" : (fileType === "audio" ? "🎵 Audio" : "📎 Document")}\n` +
                        `📝 <b>Description:</b> ${description.substring(0, 100)}...\n` +
                        `🏷️ <b>Categories:</b> ${categoriesStr}\n\n` +
                        `✅ Users can now search for: <code>${movieName}</code>`;
                    
                    await sendMessage(chatId, successMessage);
                    
                    console.log(`✅ Movie saved: ${movieName} (${fileType})`);
                    ...
                    console.log("❌ Failed to save movie to KV");
                    await sendMessage(chatId, "❌ Failed to save movie. Please try again.");
            ...
            if (update.message && update.message.text) {
                ...
                console.log(`📝 Command: ${text} from ${message.from.id}`);
